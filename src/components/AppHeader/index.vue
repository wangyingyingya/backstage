<template>
  <div class="header">
    <a href="#/">
      <img class="logo" src="@/assets/logo1.png" width="30px" alt />
...
export default {
  data() {
    ...
    // 注意在return上面 而上面不能使用逗号结束
    return {
      // user:JSON.parse(localStorage.getItem('wyy-msm-user')),
  ...
  methods: {
    ...
    handleLogout() {
      ...
      // logout(token)
      // logout(localStorage.getItem("wyy-msm-token")).then(response => {
      //   const resp = response.data;
      //   if (resp.flag) {
      //     // 退出成功
      //     // 清除本地数据
      //     localStorage.removeItem("wyy-msm-token");
      //     localStorage.removeItem("wyy-msm-user");
      //     // 回到登录页面
      //     this.$router.push("/login");
      //   } else {
      //     this.$message({
